...
import { product as oldProduct, history as oldHistory, OldProduct } from "../db/schemas/old/schema";
...
export class SimpleMigration extends BaseMigration {
    ...
    protected async processChunk(chunk: ChunkState): Promise<ChunkResult> {
        ...
        for (let i = 0; i < products.length; i += this.config.batchSize) {
            ...
            for (const oldProd of batch) {
                try {
                    console.log(`[PRODUCT] 🔄 Iniciando procesamiento de producto: ${oldProd.externalId} (UUID: ${oldProd.uuid}) - Platform: ${oldProd.platform}, Country: ${oldProd.country}`);
                    ...
                    console.log(`[PRODUCT] 👤 Procesando proveedor para producto: ${oldProd.externalId}`);
                    ...
                    console.log(`[PRODUCT] 📦 Creando/actualizando producto: ${oldProd.externalId} con proveedor: ${providerId}`);
                    const result = await this.createOrUpdateProduct(oldProd, providerId);
                    if (!result) {
                        stats.errors++;
                        console.log(`[PRODUCT] ❌ Error al procesar producto: ${oldProd.externalId}`);
                        continue;
                    }
                    ...
                    if (created) {
                        ...
                        console.log(`[PRODUCT] ✅ Producto actualizado con ID: ${productId}`);
                    }

                    // 3. Llenar gaps en historiales
                    console.log(`[HISTORY] 📊 Iniciando procesamiento de historial para producto: ${oldProd.externalId} (ID: ${productId})`);
                    const historiesCount = await this.fillHistoryGaps(oldProd, productId);
                    stats.historiesFilled += historiesCount;
                    console.log(`[HISTORY] ✅ Historial procesado: ${historiesCount} registros agregados para producto: ${oldProd.externalId}`);

                    // 4. Migrar multimedia
                    console.log(`[MULTIMEDIA] 🖼️ Procesando multimedia para producto: ${oldProd.externalId}`);
                    const multimediaCount = await this.migrateMultimedia(oldProd, productId);
                    stats.multimediaCreated += multimediaCount;
                    console.log(`[MULTIMEDIA] ✅ Multimedia procesada: ${multimediaCount} elementos para producto: ${oldProd.externalId}`);

                    stats.processed++;
                    console.log(`[PRODUCT] ✅ Producto completamente procesado: ${oldProd.externalId} - Total stats: ${JSON.stringify({processed: stats.processed, errors: stats.errors})}`);

                } catch (error) {
                    stats.errors++;
                    console.log(`[PRODUCT] ❌ Error crítico procesando producto: ${oldProd.externalId}`);
     ...
     */
    private async createOrUpdateProvider(productData: OldProduct): Promise<string> {
        try {
            if (!productData.provider) {
                console.log(`[DEBUG] No provider data for product ${productData.externalId}, creating fallback`);
            ...
            if (typeof productData.provider === 'string') {
                try {
                    ...
                    this.logError(`Error parsing provider JSON for product ${productData.externalId}:`, parseError);
            ...
            if (existingProvider.length > 0) {
                console.log(`[DEBUG] Found existing provider with ID: ${existingProvider[0].id}`);
                ...
                if (conflictingProvider.length > 0) {
                    console.log(`[DEBUG] ExternalId ${providerExternalId} already exists in another provider ${conflictingProvider[0].id}, skipping externalId update`);
                ...
                if (existingByExternalId.length > 0) {
                    console.log(`[DEBUG] Provider with externalId ${providerExternalId} already exists with ID: ${existingByExternalId[0].id}, using existing provider`);
                ...
                console.log(`[DEBUG] Creating new provider with ID: ${providerId}, name: ${providerName}`);
                ...
                if (verifyProvider.length === 0) {
                    console.error(`[ERROR] Provider ${providerId} was not created successfully!`);
     ...
     */
    private async createFallbackProvider(productData: OldProduct): Promise<string> {
        try {
            const platformType = mapPlatformToPlatformType(productData.platform);
            let countryCode = productData.country;
            ...
            try {
                ...
                this.logError(`Cannot get platform country for fallback provider:`, error);
            ...
            if (existingFallback.length > 0) {
                console.log(`[DEBUG] Found existing fallback provider with ID: ${existingFallback[0].id}`);
            ...
            console.log(`[DEBUG] Creating new fallback provider with ID: ${providerId}`);
            ...
            if (verifyFallback.length === 0) {
                console.error(`[ERROR] Fallback provider ${providerId} was not created successfully!`);
                throw new Error(`Fallback provider creation failed for ${providerId}`);
            }
            
            console.log(`[DEBUG] Fallback provider created successfully: ${providerId}`);
            this.logError(`Created fallback provider for product ${productData.externalId}`, providerId);
            return providerId;

        } catch (error) {
            this.logError(`Critical error creating fallback provider for product ${productData.externalId}:`, error);
     ...
     */
    private async createOrUpdateProduct(productData: OldProduct, providerId: string): Promise<{ productId: string; created: boolean } | null> {
        try {
     ...
     */
    private async fillHistoryGaps(productData: OldProduct, productId: string): Promise<number> {
        try {
            console.log(`[HISTORY] 🔍 Verificando historial existente para producto: ${productData.externalId} (ID: ${productId})`);
            ...
            console.log(`[HISTORY] 📅 Fechas existentes en nueva DB: ${existingHistories.length} registros`);
            ...
            console.log(`[HISTORY] 📅 Fechas disponibles en DB antigua: ${allOldHistories.length} registros`);
            ...
            if (missingDates.length === 0) {
                console.log(`[HISTORY] ✅ No hay fechas faltantes para producto: ${productData.externalId}`);
                return 0;
            }

            console.log(`[HISTORY] 📊 Fechas faltantes encontradas: ${missingDates.length} para producto: ${productData.externalId}`);
            ...
            console.log(`[HISTORY] 📋 Datos históricos obtenidos: ${missingHistoriesData.length} registros para producto: ${productData.externalId}`);
            ...
            if (historiesToInsert.length > 0) {
                ...
                console.log(`[HISTORY] 💾 Insertando ${historiesToInsert.length} registros de historial para producto: ${productData.externalId}`);
                await productsDb.insert(histories).values(historiesToInsert);
                console.log(`[HISTORY] ✅ Historial insertado exitosamente para producto: ${productData.externalId}`);
            }

            return historiesToInsert.length;

        } catch (error) {
            console.log(`[HISTORY] ❌ Error procesando historial para producto: ${productData.externalId}`);
     ...
     */
    private async migrateMultimedia(productData: OldProduct, productId: string): Promise<number> {
        try {
            ...
            if (typeof productData.gallery === 'string') {
                try {
                    ...
                    gallery = Array.isArray(parsedGallery) ? parsedGallery : [parsedGallery];
                } catch (parseError) {
                    this.logError(`Error parsing gallery JSON for product ${productData.externalId}:`, parseError);
                    return 0;
                }
            ...
            const multimediaItems = gallery
                .filter((item: MediaItem) => item && (item.url || item.originalUrl || item.ownImage || item.sourceUrl))
                .map((item: MediaItem, index: number) => {
                    // Priorizar las URLs en este orden: url > ownImage > sourceUrl > originalUrl
                    let url = item.url || item.ownImage || item.sourceUrl || item.originalUrl;
